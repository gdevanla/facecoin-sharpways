...
client = Client(
    ...
    #callback_url="oob"
    callback_url="http://thawing-savannah-5714.herokuapp.com/ocb?method=addmodel"
)
...
def ocb():
    ...
    return redirect(url_for(request.args.get('method')))

@app.route("/addmodel", methods=['GET'])
def addmodel():
    if 'oauth_verifier' not in session:
        print 'Calling auth'
        client.callback_url = "oob" #client.callback_url % ('addmodel')
        auth()
